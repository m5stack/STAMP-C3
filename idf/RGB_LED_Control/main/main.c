...
#ifdef STAMP_C3
uint8_t PINS[] = {4, 5, 6, 7, 8, 10, 1, 0, 20, 9, 18, 19};
#define BTN_PIN 3
#endif
...
#define brill 20           // inside main loop: max value for brilliance
...
void app_main(void) {
    ...
    led_strip_config_t strip_config =
        LED_STRIP_DEFAULT_CONFIG(1, (led_strip_dev_t)config.channel);
    ...
    ESP_LOGI("Main", "MAC: %X%X%X%X%X%X", base_mac_addr[0], base_mac_addr[1],
             base_mac_addr[2], base_mac_addr[3], base_mac_addr[4],
             base_mac_addr[5]);
    ...
    ESP_LOGI("Main", "Initial LED brilliance set to: r(%d), g(%d) and b(%d)",
             brill_red, brill_green, brill_blue);
    ...
    while (true) {
        if (gpio_get_level(BTN_PIN) == 0) {
            ...
            if (gpio_get_level(BTN_PIN) == 0) {
                ...
                ESP_LOGI("Main", "MAC: %X%X%X%X%X%X", base_mac_addr[0],
                         base_mac_addr[1], base_mac_addr[2], base_mac_addr[3],
                         base_mac_addr[4], base_mac_addr[5]);
                ESP_LOGI(
                    "Main",
                    "LED brilliance for r(%d), g(%d) and b(%d). Maximum: %d", r,
                    g, b, brill);
            }
        }
